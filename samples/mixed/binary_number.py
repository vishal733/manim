# https://gist.github.com/brycecorbitt/bf3432e03346ea798aa5853bc08832e3
# https://www.reddit.com/r/manim/comments/nwc4lk/mobject_for_displaying_binary_numbers/
...
import numpy as np
from samples.mixed.color_scheme import SolarizedDark as Scheme
...
class Digit(VMobject):
    ...
    def set_value(self, val, run_time=.15) -> Animation:
        ...
        anim = Succession(
            ...
            ApplyMethod(self.wait, run_time/3),
            # Wait(run_time / 3),
...
class BinaryDigit(Digit):
    ...
    def __init__(self, value=0, power=0, digit_color=Scheme.EMPHASIZED, power_color=Scheme.SECONDARY, **kwargs):
        ...
        self.power_text = Tex(self.power_str, font=self.font, tex_to_color_map={self.power_str: self.power_color}, math_mode=True)
...
class BinaryNumber(VMobject):
    ...
    def __init__(self, num_digits, data=None, starting_power=0, digit_params: dict = None, **kwargs):
        ...
        self.data1 = self._validate_data(data)
        ...
        for i in range(num_digits):
            if self.digit_params is not None:
                dg = BinaryDigit(value=self.data1[i], power=starting_power + num_digits - 1 - i, **digit_params)
            else:
                dg = BinaryDigit(value=self.data1[i], power=starting_power + num_digits - 1 - i)
            self.digits.add(dg)

        self.result_text = EqualsDigit(value=self.base10_val(self.data1))
    ...
    def set_value(self, data, run_time=.15):
        ...
        for i in range(len(digit_anims[0].animations)):
            if digit_anims[0].animations[i].__name__ == 'wait':  # If a Wait, we know the rest of them will be Wait, so only add one
        ...
        self.data1 = data  # Don't forget to update the bit values
...
class TestScene(Scene):
    def construct(self):
        ...
        change_time = lambda x: max(1 / np.power(2, np.log2(x)), 3 / self.camera.frame_rate)
