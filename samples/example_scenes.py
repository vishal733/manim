...
DETAILS = ["notes/99_misc1.py", "Shapes"]
DETAILS = ["notes/99_misc1.py", "Shapes2"]
DETAILS = ["notes/99_misc1.py", "MakeText"]
DETAILS = ["notes/99_misc1.py", "Equations"]
DETAILS = ["notes/99_misc1.py", "Graphing"]
DETAILS = ["notes/99_misc1.py", "Test"]
DETAILS = ["notes/2_warptron.py", "PendulumChaos10"]
DETAILS = ["example_scenes.py", "RotationUpdater"]

# General
# self.add, self.remove, self.wait(1), self.play(<animation>, run_time=3)

# absolute positioning -- to_edge, to_corner
# releative positioning -- move_to
# rotate(PI/2)...
# Rotate about point -- rotate(PI/2, about_point = point)
# flip(UP)
...
class Placement(Scene):
    def construct(self):
        txt = Text("Trial Text")
        txt.to_corner(UL, buff=0.1)
        self.add(txt)

        txt2 = Text("Second Text")
        txt2.move_to(np.array([0, 2, 0]))
        self.add(txt2)
        self.wait(2)
        txt2.move_to(np.array([2, 2, 0]))
        ...
        self.play(Write(txt3), Write(txt4))
        self.wait(2)
        txt3.rotate(PI/2)

        # self.play(txt2.animate.move_to(np.array[0, 3, 0]))
...
class Vishal3(Scene):
    def construct(self):
        text = Text("text").scale(1)
        # self.play(Write(text))
        self.play(FadeInFromPoint(text, np.array([-5, 0, 0])))
        self.play(VFadeOut(text), run_time=3)
        ...
        curr = VGroup(text, sq, sq2, circle)
        self.add(curr)
        self.play(curr.animate.shift(RIGHT))
        self.wait()


class ManimCELogo(Scene):
    def construct(self):
        self.camera.color = "#ece6e2"
        self.color = "#ece6e2"
        logo_green = "#87c2a5"
        logo_blue = "#525893"
        logo_red = "#e07a5f"
        ...
        circle = Circle(color=logo_green, fill_opacity=1, stroke_opacity=0).shift(LEFT)
        square = Square(color=logo_blue, fill_opacity=1, stroke_opacity=0).shift(UP)
        triangle = Triangle(color=logo_red, fill_opacity=1, stroke_opacity=0).shift(RIGHT)
...
class PointMovingOnShapes(Scene):
    def construct(self):
        ...
        self.play(Rotating(dot, about_point=[2, 0, 0], axis=IN), run_time=1.5, rate_func=linear)
        self.wait()
...
class Vishal2(Scene):
    ...
    def construct(self):
        top_y = 0.5
        bot_y = -0.5
        line = Line([-3, top_y], [3, top_y])
        self.add(line)
        line2 = Line([-3, bot_y], [3, bot_y])
        self.add(line2)
        line = Line([-1, top_y], [-1, bot_y])
        self.add(line)
        line = Line([1, top_y], [1, bot_y])
        self.add(line)
...
class Vishal(Scene):
    ...
    def construct(self):
        ...
        sphere = Sphere(radius=1, opacity=0.2, color=BLUE)
        # sphere.set_fill(BLUE, opacity=0.5)
        sphere.move_to((0, 0, 2))
        self.play(
            ShowCreation(sphere, run_time=3),
        )

        self.play(Rotate(sphere, PI / 2), run_time=2)
        # self.play(ShrinkToCenter(sphere), run_time=2)
        self.play(sphere.animate.move_to(axes.c2p(0, 0, 1)))
        self.play(
            frame.animate.increment_gamma(10 * DEGREES),
            run_time=3
        )
        ...
        # self.play(ShowCreation(square), run_time=5)  # animate the creation of the square
        # self.play(Transform(square, circle), run_time=4)  # interpolate the square into the circle
...
class AnimatingMethods(Scene):
    def construct(self):
        grid = Tex(r"\pi").get_grid(10, 15, height=4)
        # grid = Tex(r"A").get_grid(10, 15, height=4)
        self.add(grid)

        # You can animate the application of mobject methods with the
        # ".animate" syntax:
        self.play(grid.animate.shift(LEFT+DOWN), run_time=1)
        ...
        def my_func(x):
            return 2*x
        # self.play(grid.animate.apply_function(my_func), run_time=5)
...
class TextExample(Scene):
    def construct(self):
        ...
        VGroup(text, difference).arrange(DOWN, buff=2)
        self.play(Write(text))
...
class GraphExample(Scene):
    def construct(self):
        ...
        h_line = always_redraw(lambda: axes.get_h_line(dot.get_right()) if (dot.get_x() < 0) else axes.get_h_line(dot.get_left()))
        v_line = always_redraw(lambda: axes.get_v_line(dot.get_bottom()))
...
if __name__ == '__main__':
    ...
    path_file = os.path.join("/work/animation/manim/", DETAILS[0])
